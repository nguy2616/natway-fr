<template>
  <div class="natway-refuse pa-6 pr-4">
      ...
      <a-row class="fixed-bot mt-10 px-6" type="flex" justify="center" style="background: white">
    <a-col span="22">
          <a-form class="info mt-8">
                <a-form-item class="mb-3">
          <a-button shape="round" type="auth" block @click="(e)=> { $router.push('/npnatway-chart') }" >
            {{ $t('auth.Return') }}
          </a-button>
                </a-form-item>
            <a-form-item>
...
export default {
  ...
  data() {
    return {
      charter: this.$store.state.auth.rfNaturo
    }
  },
  mounted() {
    window.scrollTo(0, 0)
  ...
  methods: {
    getCharterContents() {
      this.$store.dispatch('auth/getCharterContents', 4)
        .then(() => {
          this.charter = this.$store.state.auth.rfNaturo
        })
    },
    cancelRegistration() {
