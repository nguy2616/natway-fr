...
export const timestampToTime = timestamp => {
  ...
  const nowDate = dayjs.unix(now.toString().length === 13 ? now / 1000 : now).format('MM/DD')

  let date = dayjs.unix(timestamp.toString().length === 13 ? timestamp / 1000 : timestamp).format('MM/DD')
  if (date === 'Invalid date') {
    date = ''
  }
  return nowDate === date
    ? dayjs.unix(timestamp.toString().length === 13 ? timestamp / 1000 : timestamp).format('HH:mm')
    : date
}

export const timestampToDateString = timestamp => {
  return dayjs.unix(timestamp.toString().length === 13 ? timestamp / 1000 : timestamp).format('LL')
}

export const timestampFromNow = timestamp => {
  const date = dayjs.unix(timestamp.toString().length === 13 ? timestamp / 1000 : timestamp).format('YYYY-MM-DD HH:mm:ss')

  // console.log(dayjs().from(dayjs('1990-01-01'))) // in 31 years
  // console.log(dayjs().from(date)) // in 11 hours
  // console.log(dayjs().from(date, true)) // 11 hours
  // console.log(dayjs().to(date)) // 11 hours ago
  // console.log(dayjs().fromNow()) // a few seconds ago
  // console.log(dayjs().toNow()) // a few seconds ago
